...
with st.expander("ℹ️ Инструкция: как подготовить шаблон и таблицу", expanded=False):
    tab1, tab2, tab3 = st.tabs(["📝 Шаблон документа", "📊 Таблица с данными", "🚀 Результат"])
    
    with tab1:
        st.markdown("#### Шаг 1. Откройте ваш документ Word")
        st.markdown("Поддерживаются форматы `.doc` и `.docx`.")
        st.markdown("#### Шаг 2. Вставьте переменные в нужных местах")
        st.markdown("""
Оберните название переменной в двойные фигурные скобки `{{ }}`.  
Название переменной **должно совпадать** с заголовком колонки в таблице.
""")
        st.info("💡 Формат скобок можно изменить в боковой панели «Настройки».")
        
        st.markdown("**Пример текста в шаблоне:**")
        st.code(
            "Прошу принять меня на работу в НИУ ВШЭ\n"
            "на должность {{Должность}}\n"
            "с {{Дата приема}} по {{Дата окончания}}.\n"
            "Контактный телефон: {{Телефон}}",
            language=None
        )
        st.success("✅ Не переживайте, если Word «разорвет» скобки невидимым форматированием — программа справится автоматически!")
    
    with tab2:
        st.markdown("#### Подготовьте Excel (`.xlsx`) или CSV (`.csv`) файл")
        st.markdown("""
- **Первая строка** — заголовки колонок (названия переменных **без скобок**).
- **Каждая последующая строка** — данные одного сотрудника.
""")
        st.markdown("**Пример таблицы:**")
        example_df = pd.DataFrame({
            "ФИО": ["Иванов Иван Иванович", "Петрова Анна Сергеевна"],
            "Должность": ["стажер-исследователь", "аналитик"],
            "Телефон": ["+7 999 123-45-67", "+7 999 765-43-21"],
            "Дата приема": ["«01» сентября 2025 г.", "«15» октября 2025 г."],
        })
        st.dataframe(example_df, use_container_width=True, hide_index=True)
        st.warning("⚠️ Убедитесь, что названия колонок точно совпадают с переменными в шаблоне (с учётом регистра и пробелов).")
    
    with tab3:
        st.markdown("#### Что вы получите")
        st.markdown("""
После нажатия кнопки **«Создать ZIP-архив»** программа:
1. Возьмёт каждую строку таблицы.
2. Подставит значения во все шаблоны.
3. Сложит документы в **именную папку** (по умолчанию — из колонки `ФИО`).
""")
        st.code(
            "📦 documents.zip\n"
            "├── Иванов Иван Иванович/\n"
            "│   ├── Заявление на прием.docx\n"
            "│   ├── Согласие.docx\n"
            "│   └── Лист ознакомления.docx\n"
